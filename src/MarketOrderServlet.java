...
@WebServlet(name = "MarketOrderServlet", value = "/MarketOrderServlet")
public class MarketOrderServlet extends HttpServlet {
    ...
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //try{
        try {
            Market oneMarket = marketMapper.selectById(Integer.parseInt(data.getString("marketId")));
            MarketOrder newOrder = new MarketOrder(oneMarket.getMarketId(),oneMarket.getMarketUserId(),data.getString("deliverUserId"),oneMarket.getMarketTime(),oneMarket.getMarketPrice(),oneMarket.getMarketContact(),oneMarket.getMarketTags(),oneMarket.getMarketInfo());
            int res = marketMapper.insertOrder(newOrder);
            if (res == 1) {
                res = marketMapper.deleteById(oneMarket.getMarketId());
                if(res==1) {
