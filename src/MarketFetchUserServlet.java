...
@WebServlet(name = "MarketFetchUserServlet", value = "/MarketFetchUserServlet")
public class MarketFetchUserServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            List<Market> oneMarket = marketMapper.selectByUserId(userId,8*(page-1),8);
            if (oneMarket != null) {
                String json = JSON.toJSONString(oneMarket);
                out.print(json);
            } }finally {
            sqlSession.commit();
            //sqlSession.rollback();
