...
@WebServlet(name = "DiningFetchUserOrderServlet", value = "/DiningFetchUserOrderServlet")
public class DiningFetchUserOrderServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            List<DiningOrder> oneDining = diningMapper.selectByUserOrderId( userId,8*(page-1),8);
            if (oneDining != null) {
                List<Dining> res = new ArrayList<>();
                for (DiningOrder diningOrder: oneDining
                ) {
                    User deliver = userMapper.selectById(diningOrder.getDeliverUserId());
                    res.add(new Dining(diningOrder.getDiningId(),deliver.getUserName(),diningOrder.getDeliverUserId(),diningOrder.getDiningName(),diningOrder.getDiningTime(),diningOrder.getDiningPrice(),deliver.getPhoneNum(),diningOrder.getDiningTags(),diningOrder.getDiningInfo()));
                }
