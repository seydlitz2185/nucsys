...
@WebServlet(name = "MarketProxyPickCancelServlet", value = "/MarketProxyPickCancelServlet")
public class MarketProxyPickCancelServlet extends HttpServlet {
    @Override
    protected void service(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        //try{
        try {
            MarketOrder marketOrder = marketMapper.selectByOrderId(marketId);
            if (marketOrder != null) {
                User hostUser = userMapper.selectById(marketOrder.getHostUserId());
                if(hostUser != null) {
                    Market rollbackMarket = new Market(hostUser.getUserName(),marketOrder.getHostUserId(),marketOrder.getMarketTime(),marketOrder.getMarketPrice(),marketOrder.getMarketContact(),marketOrder.getMarketTags(),marketOrder.getMarketInfo());
                    int res = marketMapper.deleteOrderById(marketId);
                    int res2 = marketMapper.insertNewMarket(rollbackMarket);
                    if(res == 1 && res2 == 1){
