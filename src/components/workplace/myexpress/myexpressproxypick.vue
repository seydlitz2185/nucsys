...
import myExpressProxyPickGrid from "./myExpressProxyPickGrid.vue"
...
onMounted(() => {
  ...
  let fetch={userId: JSON.parse(localStorage.getItem("login")).id,offset: page.value};
      ...
      axios.post(
         "http://localhost:8082/ExpressOrderFetchUserServlet",params)
         .then(function(resp){
          let res =  resp.data;
          /*
          message.info(
...
<template>
     ...
     <n-grid class="grids"  cols="4 300:4 300:4" :x-gap="24" :y-gap="24" item-responsive="true" >
      <n-gi>
        <myExpressProxyPickGrid :msg=0  />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=1 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=2 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=3 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=4 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=5 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=6 />
      </n-gi>
      <n-gi>
        <myExpressProxyPickGrid :msg=7 />
      </n-gi>
    </n-grid>
    <n-divider />
    <n-space>
