...
import myMarketOrderGrid from "./myMarketOrderGrid.vue"
...
onMounted(() => {
  let fetch={userId: JSON.parse(localStorage.getItem("login")).id,offset: page.value};
      ...
      axios.post(
         "http://localhost:8082/MarketFetchUserOrderServlet",params)
         .then(function(resp){
          let res =  resp.data;
          /*
          message.info(
...
<template>
     ...
     <n-grid  class="grids" cols="4 300:4 300:4" :x-gap="24" :y-gap="24" item-responsive="true" >
      <n-gi>
        <myMarketOrderGrid :msg=0  />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=1 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=2 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=3 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=4 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=5 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=6 />
      </n-gi>
      <n-gi>
        <myMarketOrderGrid :msg=7 />
      </n-gi>
    </n-grid>
    <n-divider />
    <n-space>
